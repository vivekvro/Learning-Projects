...
class featureEngineering(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        # # ---------- total area ----------
        # if {'sqft_living', 'sqft_basement'}.issubset(X.columns):
        #     X['totalarea'] = X['sqft_living'] + X['sqft_basement']

        # # ---------- house size ----------
        # if 'sqft_living' in X.columns:
        #     X['housesize'] = X['sqft_living']

        # # ---------- space utilization ----------
        # if {'sqft_living', 'sqft_lot'}.issubset(X.columns):
        #     X['space_util'] = (
        #         X['sqft_living'] / X['sqft_lot'].replace(0, np.nan)
        #     )
